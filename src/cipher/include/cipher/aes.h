...
namespace leaf {
	...
	inline void sub_bytes(var_unsigned& state) {
		for (auto& u: state.data)
			u = sub_bytes(u);
	}

	inline void inv_sub_bytes(var_unsigned& state) {
		for (auto& u: state.data)
			u = inv_sub_bytes(u);
	}
	...
	inline uint32_t rotation_left(uint32_t val) {
		return val << 8 | val >> 24;
	}

	inline void rotation_left(var_unsigned& state, std::size_t row, std::size_t shift) {
		uint32_t mask = ~(0xff << (4 - row - 1) * 8);
		uint8_t t[shift];
	...
	/**
	 *
	 * @tparam N_k number of uint32_t of cipher key
	 * @tparam N_b number of uint32_t of state
	 * @tparam N_r number of rounds
	 */
	template<std::size_t N_k, std::size_t N_b, std::size_t N_r>
	class aes {
	public:
		static constexpr std::size_t
		key_schedule_units = N_b * (N_r + 1),
		key_units = N_k,
		state_units = N_b;
		...
		static void inv_shift_rows(var_unsigned& state) {
			rotation_left(state, 1, N_b - 1);
			rotation_left(state, 2, N_b - 2);
			rotation_left(state, 3, N_b - 3);
		}

		static void mix_columns(var_unsigned& state) {
		...
		static void inv_mix_columns(var_unsigned& state) {
			for (std::size_t i = 0; i < N_b; ++i) {
				uint8_t b_0 = state.value<uint8_t>((N_b - i - 1) * 4 + 3),
						b_1 = state.value<uint8_t>((N_b - i - 1) * 4 + 2),
						b_2 = state.value<uint8_t>((N_b - i - 1) * 4 + 1),
		...
		static void add_round_key(var_unsigned& state, const var_unsigned& key_schedule, std::size_t round) {
			for (std::size_t i = 0; i < N_b; ++i)
				state[state.data_units() - i - 1] ^= key_schedule[key_schedule.data_units() - (N_b * round + i) - 1];
		}

		static void cipher(var_unsigned& val, const var_unsigned& key_schedule) {
		...
		static void inv_cipher(var_unsigned& val, const var_unsigned& key_schedule) {
			add_round_key(val, key_schedule, N_r);
		...
		static void key_expansion(const var_unsigned& key, var_unsigned& key_schedule) {
	...
	using aes_128 = aes<4, 4, 10>;
	using aes_256 = aes<8, 4, 14>;
}
