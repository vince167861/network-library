...
namespace leaf {
	...
	namespace sha_2 {
		template<
				...
				std::size_t D,
				auto ext = (D + 65) % block_size,
				auto blocks = (D + 65) / block_size + 1,
				auto B = block_size * blocks>
		fixed_unsigned<B> padding(const fixed_unsigned<D>& other) {
			fixed_unsigned<B> ret{other};
			if constexpr (ext)
				ret <<= block_size - ext + 65;
			ret.set(block_size - ext + 64, true);
			ret.set(fixed_unsigned<64>{D});
			return ret;
		}
		...
		var_unsigned padding(const var_unsigned& val) {
			...
			ret.set(fixed_unsigned<64>{val.bits()});
			return ret;
		}
	...
	public:
		static fixed_unsigned<256> hash(const var_unsigned& val) {
			...
			fixed_unsigned<256> ret;
			for (auto i : H) {
				ret <<= sizeof i * 8;
				ret.set(fixed_unsigned(i));
			}
			return ret;
		}
	};
	...
	public:
		static fixed_unsigned<384> hash(const var_unsigned& val) {
			...
			fixed_unsigned<384> ret;
			for (uint8_t i = 0; i < 6; ++i) {
				ret <<= 64;
				ret.set(fixed_unsigned(H[i]));
			}
			return ret;
		}
	};
