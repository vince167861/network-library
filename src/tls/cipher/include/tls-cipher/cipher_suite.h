...
namespace leaf::network::tls {
	...
	public:
		...
		virtual std::string HMAC_hash(std::string_view data, std::string_view key) const = 0;

		/**
		 * Key schedule function.
		 */
		std::string HKDF_expand(std::string_view key, std::string_view info, std::size_t length) const;
		...
		static std::string HKDF_info(std::string_view label, std::string_view context, std::uint16_t length);

		/**
		 * Key schedule function.
		 */
		std::string derive_secret(std::string_view key, std::string_view label, std::string_view msg) const;
