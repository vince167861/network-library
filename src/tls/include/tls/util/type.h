...
template<>
struct std::formatter<network::tls::content_type_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::content_type_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::ext_type_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::ext_type_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::protocol_version_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::protocol_version_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::named_group_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::named_group_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::cipher_suite_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::cipher_suite_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::signature_scheme_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::signature_scheme_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::alert_level_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::alert_level_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::alert_description_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::alert_description_t, std::format_context&) const;
};

template<>
struct std::formatter<network::tls::psk_key_exchange_mode_t> {

	constexpr auto parse(const std::format_parse_context& ctx) {
		return ctx.begin();
	}

	std::format_context::iterator format(network::tls::psk_key_exchange_mode_t, std::format_context&) const;
};
